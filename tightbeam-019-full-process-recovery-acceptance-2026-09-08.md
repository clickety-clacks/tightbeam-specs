...
## Test-support recipe from bounded source inspection

The existing probe-only `priv/harness_cli/fixture` must remain unchanged. A source check against immutable reliability commit `a335f1cafa6df0fe397008cc512f5b1fda92677d` confirms a dedicated turn-capable ACP test fixture can exercise ordinary application startup, lanes and scheduler recovery without production changes. This is a recipe to implement and verify, not executed acceptance evidence.

Startup checks adapters for every registered harness before credential readiness. Prepare arena-only package shapes for all of them using the existing test pattern in `Harness.Support.install_fake_adapter!/2` and each module's public `conformance_vectors()["ensure_adapter"]` local-present profile. Those profiles supply binary/package/bundle names, exact version and accepted patched bytes. Inactive Claude/Codex binaries must fail visibly if invoked and receive no credentials. This prevents missing packages from invoking npm; it is not provider installation or compatibility evidence.

Only Fixture receives a dedicated executable under `test/support`, projected as the arena's `adapters/node_modules/.bin/fixture-acp`. Preserve its expected `@agentclientprotocol/fixture-acp` version `1.0.0` and accepted `dist/fixture.js` anchor. Existing `provider_additivity_test.exs:35-61` demonstrates synthetic fixture onboarding through ordinary begin/staging/finish with literal `fixture-provider-credential`, entirely within the marked arena. No live credential is read or reused.

The existing test build enables Fixture. Select the isolated base/port, `default_harness: :fixture`, `fixture-model` and `autostart: true`, then start the real application. The dedicated executable implements only the exercised ACP contract from `acp_adapter_test.exs:164-415`: initialization, session create/load, truthful model/effort configuration, mode, prompt output with a unique agent-message marker and `end_turn`, plus cancellation/closure where exercised. Keep the supervisor, scheduler, lane and recovery functions intact.

The test must still prove real OS-process death/restart and a completed queued consumer turn with its marker and durable evidence. It proves Tightbeam recovery using a deterministic provider double, not real-provider behavior. The external lead supplied these concrete seams to the existing composer in `w_67962617-7cee-4e24-b520-dcf1e92c4844`; existing source/test custody remains.

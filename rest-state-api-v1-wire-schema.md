...
- `OrgFaultObservation`:
  `{id:S, host:S, harness:S, provider:S, grantGeneration:I, axis:S, outcome:S,
...
- `OrgFaultStaleEvidence`:
  `{id:S, terminalFaultId:S, host:S, harness:S, provider:S,
  grantGeneration:I, axis:S, outcome:S, capDeadReasonClass:S|null,
  causeCode:S, observedAt:I, sourceOrdinal:I, sourceKind:S, sourceId:S, principal:S,
...
- `OrgFaultAssignmentImpact`:
  `{faultId:S, provider:S, faultState:S, transitionVersion:I, cleared:B,
  obligationKind:S|null, linkCauseCode:S, clearedCauseCode:S|null}`.
...
`resource:"org faults"`. The detail envelope for
`GET /api/assignments/:assignmentId/org-fault-impacts` uses
`resource:"assignment org-fault impacts"`.
...
| assignment org-fault impacts | assignmentId, dependencyVersion | — | — | impacts `A<O<OrgFaultAssignmentImpact>>` | none |
...
- org fault kind: `configured_harness_provider_unavailable`; org fault state:
  ...
  Org-fault source IDs match `^src_[A-Za-z0-9]+$`. Grant generations, source
  ordinals, transition versions, and terminal source ordinals are positive integers.
  ...
  evidence. `OrgFaultAssignmentImpact.clearedCauseCode` is non-null exactly
...
- Assignment org-fault impacts sort by `(provider,faultId)`.

...
## Live-base guard and usable upgrade acceptance

Owner `att_8b3f0dae-7e27-488e-a631-d8830a205658` accepts the bounded guard proposal `/home/mike/.tightbeam/work/5b7bf49feee5/live-base-guard-proposal.md`, SHA256 `86b056e812bb8435507be875b40a52a94537980228137b81eec8eeb81364a8c1`. The lead read the proposal and actual acceptance; an independent external applicability review found it consistent with the selected both-line guard and existing upgrade-safety requirement. This is design acceptance, not implemented behavior or an installation grant.

Different payload identities require exact canonical-base/source/target transition input; markerless adoption also binds the expected schema stamp. An already-authorized installer may prepare and carry that input as part of its approved operation. Do not turn this mechanical input into an invented fresh human approval at every step. Current installer support remains unverified.

Release acceptance must bind package identity generation and verification to the actual Linux/macOS assembly and development paths, with an explicit digest input list covering behavior-bearing bytes. The same owned integration route must provide a supported startup transition handoff. Demonstrate the first real supported 0.1.8-to-0.1.9 migration using explicit unmarked adoption and preservation of work, decisions and wakes; ordinary restarts after success; safe handling of stale transition arguments; failure between migration and marker replacement; exact compatible reverse transition; and refusal of unsupported schema rollback. Historic binaries without this guard cannot be retroactively protected.

The guard must refuse before prohibited writes on every persistent DB/Schema and early identity path. Prove read-only schema inspection does not change DB, WAL, SHM or identity files, and resolve actual concurrent-startup ownership without assuming immutable SQLite input. Original worktree/coder isolation obligations remain. Shipping the guard without a usable authorized upgrade procedure fails release acceptance, even if isolated guard tests pass.

Existing f18/dd36/REST owners retain their source allocations and shared-write order. Lead handoff `w_f3fa1b58-96a1-474e-8037-a73b296a73d0` asks the coordinator to bind package/procedure ownership and exact acceptance cases. It does not establish that the handoff was consumed or authorize runtime, public export or installation actions.

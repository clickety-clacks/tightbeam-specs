...
This correction changes only the read validator and the preflight clauses that mistook unread records for current rendered-content state. `surf_ace_read_response_ok` remains strict after each push. New `surf_ace_current_read_response_ok` accepts either a valid current delta or the exact current-empty, no-loss shape for baseline and restoration checks. `capture-pane` remains the content-identity and semantic-restoration oracle. The self-test consumes the exact Red response in `fixtures/preflight-read-current-empty.json` and rejects stale, lossy, wrong-scope acknowledgement, and malformed nonempty variants.

...
The current-empty read correction consumes refusal `art_63b949fd` at SHA-256 `e1304d59a8f479e0be2698283a7101a1b4a78357602849838ae5ab7043e3500f`. Its exact response had top-level `ok: true`, the expected scope, `cacheStatus: current`, `consumableLoss: null`, `acknowledgement: null`, and no unread records while its same-controller `capture-pane` returned the expected current rendered content. The product initializes a newly admitted controller's existing-scope cursor at the scope tail, so pre-existing content is not replayed as unread. The focused fixtures preserve the exact read and an image-free semantic projection of that exact capture; the self-test rejects wrong content, revision, controller, stale cache, loss, or failed exit.

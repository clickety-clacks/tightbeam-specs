...
Close each bounded evaluation with the evidence available, a dated judgment and an owned next action. An inconclusive evaluation can be complete; it does not establish that the treatment succeeded. Keep any promised observation or delivery obligation owned separately.

The orchestrator arranges a review at deployment. For this initial 0.1.9 trial, propose a first review one week after the first confirmed exposure of the treatment being assessed, with earlier attention to a material regression. This is a scheduling choice for this trial, not a new systemwide gate or fixed quota. Review whatever evidence exists then; if the sample is weak, record inconclusive and choose a useful next review. Do not wait indefinitely for enough successful cases. An unavailable owner triggers ordinary ownership recovery.

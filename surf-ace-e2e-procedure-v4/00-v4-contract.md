...
- **Admission evidence:** A durable reference that names the exact surface, controller fixture identity, fixture expiry and cleanup contract, validity across restart/recovery boundaries, and one permitted admission basis.
...
2. The operator targets a surface only after its admission evidence passes.
...
7. A `pair.request` `capability_mismatch` ends the current fixture before mutation.
...
2. Given durable evidence that one discovered surface is already lockless, when the operator records the exact surface/controller-fixture binding, fixture expiry and cleanup contract, and restart validity, then the checklist can admit that surface as the primary surface.
...
6. Given a `pair.request` response with `capability_mismatch`, when the operator reaches the first mutation boundary, then the run records no mutation, performs terminal fixture/state cleanup, classifies endpoint/procedure readiness, and routes a fresh fixture without retry.

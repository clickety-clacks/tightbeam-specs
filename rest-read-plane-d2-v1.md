...
- Composition authority: current canonical `rest-state-api-v1.md`, its
  normative `rest-state-api-v1-wire-schema.md` companion, and
  `event-firehose-v1.md` on the parent of this composition. Later canonical
  clauses govern over the historical review baseline.
- Owner ruling `att_a4ca52cf-87bc-48f6-a161-2d9e6132283d` requires
  resource-specific canonical REST seams, existing D1 Router/M1 handling,
  and the exact current Toplines R9 dependency set. The accompanying REST
  R3b correction removes stale adapter names without changing public bytes.
...
- Accepted Toplines closure: canonical commit
  `05d08b8af74a877d4dabe3dcba8250787d5d430e`, REST SHA-256
  `4f9c5ca7e5c0c429b24d5011c68137f59e734f4bc5494fbbda27bca50be142ee`,
  completion `att_6a0da1aa-9bbe-43fb-8c8e-9c5be21aeda4`, clean verdict
  `att_a612059b-230d-4b0c-be05-706fbc3b022d`, report `art_55ed30e7`.
- Accepted ExecutionMap closure: canonical commit
  `0139d9a71180a7175965473fade9b183d2b57601`, REST SHA-256
  `d1af5567ec4ad6bd4cf36776790892f87c26fa62eb73a7cd0a88f4804079b88f`,
  completion `att_c6a21d4d-af6a-4baa-b30b-130fb4ad9b57`, clean verdict
  `att_22ba96ea-abbb-4fd7-b173-d35efa92900b`, report `art_f5907baf`.
  Its coupled wire SHA-256 is
  `2be26c6a5d67e3b7243ad5b3059b57f41c161d55b55f8e8433a1ef1ce4710bb4`;
  firehose SHA-256 is
  `f7ff75a2b3559208bcee5a50be03414a0d539328300a1f519118dd168101830d`.
  These closed work items supply historical prerequisite proofs; current
  canonical semantics must still be preserved. Their closure supplies no
  D1 implementation or canonical-green product proof.
...
- **D1 foundation**: existing `Tightbeam.Wire.Router` request parsing, cursor
  handling, and envelope handling plus D1's authentication, status, error,
  and `Cache-Control: no-store` contract. These are responsibilities, not
  requirements to introduce new modules or functions.
- **Query owner**: the named resource module that owns collection and detail
  selection through one canonical shared query seam. Collection and detail
  selection use that same seam where the resource has both routes.
- **Public serializer**: the resource-specific canonical item function.
...
2. D1's final reviewed integration supplies its existing Router/M1 handling.
   The owner ruling above replaces the historical assumption that D1 would
...
3. Existing D1 Router handling is the D2 GET transport seam. Route clauses do not
...
Each GET enters the existing `Tightbeam.Wire.Router` handling. The adapter
calls the resource's canonical shared query, invokes the listed
`Tightbeam.StateVisibility` predicate, and passes rows to its one public
serializer. Existing D1 handling owns request parsing, cursors, and envelopes.
D2 adds no `rest_read/3`, REST cursor/envelope module, or sibling
`StateResources.list_*` API to satisfy historical D2 names.

In the table, collection selection means collection input to the existing
shared resource query, not a new function. Named `query_*` entries serve
collection and detail input where both apply. R3b owns the four ExecutionMap
queries; R6c owns the sessions and Toplines queries and serializers. Those
resource-specific functions are not wrapped in a second StateResources
projection. Query input contains the resolved principal and normalized route
identifiers, admitted query values, and transport parameters, not a raw query
string.
...
| harness catalog | `GET /api/catalog/harnesses` | canonical collection selection — `Tightbeam.ModelCatalog` | `harness_catalog/1` | `harness_catalog_visible?/1` |
| sessions | `GET /api/sessions`; `GET /api/sessions/:sessionKey` | `query_public_sessions/2` — `Tightbeam.Org` (R6c) | `Tightbeam.Org.public_session_item/1` | `session_visible?/3` |
| transcript messages | `GET /api/sessions/:sessionKey/messages` | canonical collection selection — `Tightbeam.Transcript` | `message/1` | `message_visible?/3` |
...
| work items | `GET /api/work-items`; `GET /api/work-items/:id` | `query_work_item/2` — `Tightbeam.WorkItems` | `work_item/1` | `work_item_visible?/3` |
...
| assignments | `GET /api/assignments`; `GET /api/assignments/:id` | `query_assignment/2` — `Tightbeam.Assignments` | `assignment/1` | `assignment_visible?/3` |
| attests | `GET /api/attests`; `GET /api/assignments/:id/attests` | canonical collection selection — `Tightbeam.Assignments` | `attest/1` | `attest_visible?/3` |
| wakes | `GET /api/wakes`; `GET /api/wakes/:wakeId` | `query_wake/2` — `Tightbeam.Wakes` | `wake/1` | `wake_visible?/3` |
| digest members | `GET /api/wakes/:wakeId/digest-members` | canonical collection selection — `Tightbeam.Wakes` | `digest_member/1` | `digest_member_visible?/3` |
| turns | `GET /api/turns`; `GET /api/turns/:seq` | `query_turn/2` — `Tightbeam.Ledger` | `turn/1` | `turn_visible?/3` |
| artifacts | `GET /api/artifacts`; `GET /api/artifacts/:artifactId` | `query_artifact/2` — `Tightbeam.Artifacts` | `artifact/1` | `artifact_visible?/3` |
| assets | `GET /api/assets`; `GET /api/assets/:assetId`; `GET /download/:assetId` | `query_asset/2` — `Tightbeam.Assets` | `asset/1` | `asset_visible?/3` |
| decision requests | `GET /api/decision-requests`; `GET /api/decision-requests/:id` | `query_decision_request/2` — `Tightbeam.Escalation` | `decision_request/1` | `decision_request_visible?/3` |
| read markers | `GET /api/read-markers`; `GET /api/read-markers/:scopeKey` | `query_read_marker/2` — `Tightbeam.ReadMarkers` | `read_marker/1` | `read_marker_visible?/3` |
| roles | `GET /api/roles` | canonical collection selection — `Tightbeam.Roles` | `role/1` | `role_visible?/3` |
| toplines | `GET /api/toplines`; `GET /api/toplines/:id` | `query_public/2` — `Tightbeam.Toplines` (R6c) | `Tightbeam.Toplines.public_item/1` | `topline_visible?/3` |
...
| facts | `GET /api/facts` | canonical collection selection — `Tightbeam.ConditionFacts` | `condition_fact/1` | `condition_fact_visible?/3` |
| critical state | `GET /api/critical-state` | canonical collection selection — `Tightbeam.CriticalLeases` | `critical_state/1` | `critical_state_visible?/1` |
| archetypes | `GET /api/archetypes`; `GET /api/archetypes/:name` | `query_archetype/2` — `Tightbeam.Archetypes` | `archetype/1` | `archetype_visible?/1` |
| guidance | `GET /api/guidance`; `GET /api/guidance/:name` | `query_guidance/2` — `Tightbeam.Identity` | `guidance/1` | `guidance_visible?/1` |
| rails | `GET /api/rails`; `GET /api/rails/:name` | `query_rail/2` — `Tightbeam.Rails` | `rail/1` | `rail_visible?/1` |
| harness processes | `GET /api/harness-processes` | canonical collection selection — `Tightbeam.HarnessProcess` | `harness_process/1` | `harness_process_visible?/1` |
| devices | `GET /api/devices`; `GET /api/devices/:deviceId` | `query_device/2` — `Tightbeam.Devices` | `device/1` | `device_visible?/1` |
...
primary-key bytes. Canonical REST R9b owns Toplines source-kind labels,
natural event-sequence versions, and visible membership extraction; R9a
owns ExecutionMap's resource and non-resource dependency entries. Those
contracts govern the entries used here without a second extraction rule.
...
A JSON collection returns HTTP 200 through existing shared envelope handling:
...
existing shared envelope handling:
...
- `lib/tightbeam/wire/router.ex`: D2 GET clauses and reuse of existing D1
  request, cursor, and envelope handling; download authorization before byte open.
- `lib/tightbeam/state_resources.ex`: shared queries and public serializers
  for its owned resources; existing serializers remain unchanged unless the canonical
...
- Existing D1 cursor and envelope handling: reuse only; D2 adds no second
  implementation or module merely to satisfy an obsolete filename.
...
22. **Toplines semantic boundary.** Given durable human-intent Toplines and
    ...
    `Tightbeam.Toplines.public_item/1` items; no ExecutionMap node or envelope appears.
23. **ExecutionMap route modes.** Given authorized telemetry, when the four
    ...
    selection routes are unpaged; no route invokes `Tightbeam.Toplines.public_item/1`.

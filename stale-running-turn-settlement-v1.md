...
Status: SPEC CANDIDATE — bounded recovery of the two supplied specimens. This
artifact is additive to `eea3e9a` and does not replace automatic adapter-death
recovery.
...
4. The authenticated principal SHALL be an admin operator represented as
   ...
   mutation.
5. The settlement decision SHALL use the lane that owns the session's live
   turn fact. The lane SHALL report one of `live`, `stale`, or `ambiguous` for
   the exact `(sessionKey, turnSeq)` before the database transition.
6. The lane SHALL report `live` when that exact sequence is its current task.
   ...
   when the lane is unavailable, or when its in-memory sequence disagrees with
   the durable running row. The verb SHALL return `turn_status_ambiguous` and
   SHALL perform no mutation. The lane SHALL report `stale` only when it has
   established that the exact target is not executing in the lane.
...
8. For a `stale` result, one serialized operation SHALL verify all of the
   following and then perform the transition: the session row exists, the
   session state is `active`, the target row belongs to that session, the
   target status is `running`, and `endedAt` is NULL.
...
15. The request's idempotency key SHALL be accepted only as an opaque bounded
    replay key. A retry after a committed settlement SHALL return the existing
    terminal row and shall not append another lifecycle event, publish another
    terminal effect, or release the queue twice. The durable turn row and its
    existing terminal lifecycle event SHALL remain the source of truth; no
    separate idempotency state is permitted.
...
2. The live/stale/ambiguous decision and the settlement CAS execute through one
   lane serialization seam. A caller SHALL NOT sample lane state, leave that
   seam, and later perform an unconditional database update.
...
2. Add an operator settlement handler that resolves the active session lane,
   asks that lane for the exact target's live/stale/ambiguous result, and
   refuses on `live` or `ambiguous`. The handler SHALL not perform a direct
   ledger mutation outside the lane seam.
3. In the stale branch, use the existing terminal transition seam with an
   ...
   guarded target update, lifecycle append, and `turn.ended` publication
   effect. It SHALL return the row's terminal result when another writer won.
...
8. **CAS race.** Given one stale target and two concurrent settlement requests,
   ...
   same one-terminal rule decides the result.
...
10. **Queue release.** Given one stale running target and one queued successor,
...
11. **Restart before commit.** Given a process loss before the settlement CAS
...
12. **Restart after commit.** Given process loss after the settlement CAS but
...
13. **Compatibility and rollback.** Given an older client or server, then the
    ...
    row is recreated.
14. **Observability.** Given one accepted request, one live refusal, one
...
15. **Deterministic replay matrix.** The acceptance suite SHALL run the above

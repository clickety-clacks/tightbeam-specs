...
- `implementation-requires-posture` and `implementation-dispatch-requires-posture`, engineering rules lines 102-124: deny coder assignment/dispatch without a work-item posture-light/heavy verdict. Selected disposition: remove both vetoes together with the explicit small-fix path, removal of heavy-by-default and blanket light ceremonies, and supervisor accountability for proportionate work. Mike explained their original purpose; preserve that benefit in the composed guidance. The token accepts heavy as readily as light and does not establish proportionality. Validate the replacement before release; evaluate actual outcomes after use.
...
The lead has selected removal of the two posture gates as part of this coherent replacement for 0.1.9. This supersedes the intermediate unresolved disposition, while preserving the correction to bare deletion. Four named vetoes are retired: the two posture rules, count-forced diagnosis and passing-test review admission. The blanket spirit gate is replaced by timely PO opportunity and ownership; review protection remains while its staffing remedy changes. The selected behavior still needs implementation review and pre-release validation, not evidence from future users before a design can be chosen. See `tightbeam-019-agent-judgment-plan.md` for the full release scope and its pinned-source limitation.

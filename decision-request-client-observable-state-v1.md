...
- **Served decision-request row**: the internal contract passed from the canonical
  visibility-filtered decision-request query to the shared R7 serializer. Its exact
  shape is `{rowSchemaRevision:1|2,item:<R7 decision-request item>}`.
  `rowSchemaRevision` is persisted row metadata and serializer input, not an R7 field,
  REST envelope field, route version, or client argument. The serializer reads this
  member; it does not derive the member from `status`.
- **Revision adoption boundary**: the one migration boundary that assigns persisted
  row-schema revisions to decision-request rows that predate this amendment. The G6
  REST read and DR2 transition paths remain unavailable until that boundary completes.
...
DR1. Each decision-request row persists `rowSchemaRevision` with value `1` or `2`
beside the R7 item. At the revision adoption boundary, the migration writes `2` for an
existing row whose status is `returned`, writes `1` for an existing row whose status
belongs to the revision-1 enum below, and refuses activation when an existing row has
any other status. The G6 REST read and DR2 transition paths activate only after each
existing row has one of those persisted values.

Creation of a row and each successful DR2 transition persist `rowSchemaRevision:2` in
the same commit as the item and its `rowVersion`; a read does not change the persisted
value.
The visibility-filtered query passes that stored revision and item as one served
decision-request row to the shared serializer. For each item in a list or detail
response, the serializer selects the status enum only from that explicit revision
input and validates before it emits any response bytes. Selection and validation are
one indivisible serializer step.

Under revision 1, the public status domain is the closed set `open`, `answered`,
`ruled`, `consumed`, `withdrawn`, and `superseded`. Under revision 2, the public status
domain is the closed set `open`, `answered`, `returned`, `ruled`, `consumed`,
`withdrawn`, and `superseded`. The serializer rejects a missing revision, a revision
other than `1` or `2`, or a status outside the enum selected by the served row. It does
not infer a revision from the status, route, REST envelope, or a caller default. A
rejection emits no partial list item, detail item, REST response, or state notice.
[AC1]
...
existing firehose recovery rules. During that rebuild, the server applies DR1 to each
served row; revision 1 and revision 2 rows can appear in one REST snapshot, and one
invalid served row fails the response before the client replaces its map. The client
keys each accepted item by its `id`, and a later notice correlates only when
`refs.decisionRequestId` equals `payload.id` and the REST item `id`.

A revision-1-only client rejects `returned` in a REST item and applies no partial
snapshot. A client that does not support a received notice `schemaVersion` discards
cached decision-request state, reports the unsupported version, and waits for a
compatible decoder before it applies another decision-request snapshot or notice.
[AC7]
...
The canonical query supplies one served decision-request row per REST item. The shared
serializer reads the persisted row-schema revision from that contract, selects the
matching closed status enum, validates the item, and only then encodes it. A list can
contain both revisions because selection occurs once per served row before any list
bytes are emitted. No route, REST envelope, status value, or serializer caller chooses
the revision during list or detail serialization. Only the served row's
`rowSchemaRevision` selects it. The same persisted value commits through the sole
mutation seam with the item and `rowVersion`.

The revision adoption boundary is the sole pre-activation initializer of persisted
row-schema revision. After activation, the decision-request transition function is the
sole mutation seam for that value.
...
function that validates DR2, writes `rowSchemaRevision:2`, increments `rowVersion`, and
hands the committed item and selected class to the existing post-commit publisher.

## Acceptance

AC1 — Public enum closure (DR1)

- Given seven served-row fixtures with `rowSchemaRevision:2` whose statuses cover
  `open`, `answered`, `returned`, `ruled`, `consumed`, `withdrawn`, and `superseded`,
  when an authorized detail request serializes each fixture, then each response
  envelope has `schemaVersion:1`, resource `decision requests`, the unchanged R7 key
  and type set, and one accepted public status.
- Given one `rowSchemaRevision:1` served row with status `open` and one
  `rowSchemaRevision:2` served row with status `returned`, when an authorized list
  request serializes both rows, then one complete response contains both unchanged R7
  items under the REST R1 envelope.
- Given the same `returned` R7 item first paired with `rowSchemaRevision:1` and then
  paired with `rowSchemaRevision:2`, when the shared serializer processes each served
  row, then the revision-1 case fails before response bytes and the revision-2 case
  emits the item. This fixture proves that the explicit revision input, not the status
  or route, selects the enum.
- Given a served row with `rowSchemaRevision:2` and status `paused`,
  `rowSchemaRevision:3` and status `open`, or no `rowSchemaRevision` and status `open`,
  when the shared R7 serializer reads it, then serialization fails and emits no partial
  item, REST response, or state notice.
- Given one existing `open` row and one existing `returned` row without persisted
  revisions, when the revision adoption boundary completes, then the first row has
  `rowSchemaRevision:1`, the second row has `rowSchemaRevision:2`, and the G6 REST read
  and DR2 transition paths activate after both writes complete.
- Given an existing row with status `paused`, when revision adoption runs, then it
  refuses activation and the G6 REST read and DR2 transition paths remain unavailable.
- Given an adopted `rowSchemaRevision:1` row, when its next successful DR2 transition
  commits, then its item, higher `rowVersion`, and `rowSchemaRevision:2` become visible
  together.
...
- Given a committed `open` to `returned` transition for `dr_123` that persists
  `rowSchemaRevision:2` and `rowVersion:9`, when an authorized client receives the
  notice and fetches
...
- Given a schema-version-2 client misses, duplicates, or reorders decision-request
  notices and REST serves one valid revision-1 row plus one valid revision-2 row, when
  the client follows the existing recovery rule and rebuilds from REST, then its map
  keyed by decision-request `id` equals the complete fresh REST snapshot and subsequent
  higher `rowVersion` notices converge by last-version-wins.
- Given a REST list contains one valid served row and one row whose persisted revision
  is `3`, when the client rebuilds, then the server emits no partial snapshot and the
  client does not replace its cached map.
- Given a revision-1-only client requests a snapshot containing a `returned` item,
  when it validates the closed revision-1 enum, then it rejects that item and applies
  no partial decision-request map; after the client has a revision-2 decoder, a fresh
  request accepts that item.
...
- Given a revision-2 notice for `dr_123` at `rowVersion:9` and a rebuilt REST item with
  `id:"dr_123"` at `rowVersion:9`, when the client correlates the notice, then it
  accepts the match only because `refs.decisionRequestId`, `payload.id`, and the REST
  item `id` are all exactly `dr_123`.
...
AC8 cite the requirements they verify. The revision-selection seam in Architecture
traces only to DR1, DR4, and DR7; the transition registry traces only to DR2 through
DR5.

...
Status: DRAFT FOR SUCCESSOR REVIEW. Authority: `wi_795a354d-3f97-4805-b472-b637c7542d54`, Mike's rewrite order `att_7ccdd6bf-48f6-46c4-86f4-f96609594e8f`, Mike's algedonic ruling `att_6b976672-6456-486a-808e-8ecbebcadb5f`, and review verdicts `att_46758d8c-b1aa-4e14-8a96-307cd5448ef3` and `att_b51166b7-2e39-4841-aac5-0dbfc518c6bc`. Canonical home: repository `clickety-clacks/tightbeam-specs`, path `substrate-native-exec-desks-v1.md`. This text replaces the changes-requested revision at `d9fd29e22b197e3eaf511e70c7a045ec64f6f178`; revisions `0767558eccfcdee974bee9d2b426c74e6f92fcc6` and `ac965ae33852d6890b6ae2624d9a899fed1722cb` are provenance only.
...
- **Activity-summary NOTE:** the single durable row for one worker and debounce window. It begins `pending` as the semantic-work claim and carries a nullable semantic-start time. It then becomes `published` with a plain observation or `failed_with_reason` with a cause. A published row records the worker, exec, covered time window, write time, transcript evidence, and prior published NOTE. An empty window says `no activity since <window-start>`.
...
The shell chooses timing from durable message kind and the existing class field: following `coordination-fabric-v1.md`, a present `fyi` or `status-query` permits the next bundle, a present `input-needed` or `blocker` requires now, and no class sends agent or user messages now while substrate notices and effort check-ins may join the next bundle. Under `att_6b976672-6456-486a-808e-8ecbebcadb5f`, `algedonic` is a deterministic exception. For a worker other than Owner Main, the routing transaction creates one ordinary send addressed to Owner Main, links the new wake to the source wake, admits that send, and records `handled` as the source outcome. The exec preserves the content and class without answering, absorbing, triaging, or reclassifying it. If the addressed worker is Owner Main, its exec delivers the source wake to that Main worker instead of creating another send. Main is the user boundary, and the fabric creates no second human carrier. This rule replaces the conflicting algedonic carrier text in `coordination-fabric-v1.md`.
...
3. **A3 — Timing inputs.** Given an unclassed agent message, an `fyi`-class user message, and an `algedonic` wake reach an exec, when the shell chooses timing, then the agent message goes now and the `fyi` message goes now or in the next bundle. Given the addressed worker is not Owner Main, its exec admits one unchanged, worker-addressed wake to Owner Main, links it to the source, and records the source as handled without intermediate judgment or a second human carrier. Given the addressed worker is Owner Main, its exec delivers the source wake to the Main worker and creates no new send.

...
Status: DRAFT FOR SUCCESSOR REVIEW. Authority: `wi_795a354d-3f97-4805-b472-b637c7542d54`, Mike's rewrite order `att_7ccdd6bf-48f6-46c4-86f4-f96609594e8f`, Mike's algedonic ruling `att_6b976672-6456-486a-808e-8ecbebcadb5f`, and review verdicts `att_46758d8c-b1aa-4e14-8a96-307cd5448ef3`, `att_b51166b7-2e39-4841-aac5-0dbfc518c6bc`, and `att_6f42c78a-6a5d-4750-b918-55053f209828`. Canonical home: repository `clickety-clacks/tightbeam-specs`, path `substrate-native-exec-desks-v1.md`. This text supersedes the reviewed changes-requested revision at `561ced6d0c7df35f5a3979b3d11185f752b32409`; revisions `7a13f97d6f418d2bf3104828619d6d6073aebf57`, `d9fd29e22b197e3eaf511e70c7a045ec64f6f178`, `0767558eccfcdee974bee9d2b426c74e6f92fcc6`, and `ac965ae33852d6890b6ae2624d9a899fed1722cb` are provenance only.
...
5. **Two names.** An exec-authored BUNDLE, NOTE, annotation, escalation, or algedonic child wake records the worker it represents and the exec that performed the action. (A10)
...
3. **A3 — Timing inputs.** Given an unclassed agent message and an `fyi`-class user message reach an exec, when the shell chooses timing, then the agent message goes now and the `fyi` message goes now or in the next bundle. Given an `algedonic` wake reaches an exec whose represented worker is not Owner Main, when routing commits, then one unchanged worker-addressed child wake is admitted to Owner Main, records the source wake, represented worker, and performing exec, and leaves the source nonterminal. Given that child later reaches `handled` or `undeliverable`, when its terminal transaction commits, then the source reaches the same terminal kind and an undeliverable source records the child's cause. Given the represented worker is Owner Main, its exec delivers the source wake to the Main worker and creates no new send.
...
10. **A10 — Attribution.** Given the exec writes a BUNDLE, NOTE, annotation, escalation, or algedonic child wake, when the row is read, then it identifies the represented worker and performing exec as separate fields.

...
2. A failed turn opens an episode only when its raw failure is classified as
   ...
   `usageLimitExceeded`. Credential onboarding and OAuth recovery remain on the
   credential recovery rail.
...
6. If no active owner Main exists, Tightbeam records
   ...
   a turn for a session row that does not exist.
...
- Nested Codex `usageLimitExceeded`, adapter-down, and model-apply-unavailable
  shapes classify; OAuth policy refusal and arbitrary model errors do not.
- A missing or retired Main receives no wake, produces named durable evidence,
  and leaves the episode open.

# Release readiness, frozen main candidates, and optional release lines — revision 5

Status: spec-approved candidate for fresh independent exact-artifact review.  
Canonical path: `specs/tightbeam/release-readiness-design-v5.md`  
Work item: `wi_3c9d3a64-15a8-457e-b2b1-429a1264f04f`  
Producer assignment: `asg_ea220e82-5a11-4706-9fd0-ffbb979dc134`

This revision replaces the decision authority of revision 4, `art_25b966e7`, SHA-256
...
This revision closes the eight findings in revision-4 review
...
digest as revision 4 or revision 5.
...
reissue key, delivery acknowledgments, and terminal actor/cause/time. Reissue appends a new
offer and changes the pending predecessor to `superseded`; it never rewrites history.
...
Its only authority is to propose a brief or evidence. It ends on explicit revocation, its
declared future expiry, or the end of its owner epoch.
...
key, state `active | retired`, and reissue policy
...
either the pending initial responsibility offer or an owner epoch, never both; the accepted
initial offer resolves to its atomically opened epoch without rewriting the initial brief.
...
8. The release tables described in revisions 3 and 4 have not shipped. Migration can add
   the consolidated revision-5 schema without translating live release rows.
...
Each optional-line push carries `eventId` and `operationId`. Its idempotency key is derived
from `(trainId, lineGeneration, targetRef, fromSha, toSha)`. The durable admission records
states `prepared | effect_pending | receipt_recorded | terminal`. Redelivery with another
event id but the same derived key converges on that operation and its first terminal run or
non-run result. Reuse of an event id with a different immutable payload is an identity
mismatch.
...
Optional-line push admission uses the envelope operation id and derives its stable
idempotency key from train, line generation, target ref, from-SHA, and to-SHA. It stores the
canonical envelope hash, all event ids observed for that operation, and the prepare/effect/
receipt/terminal history. Explicit dispatch requires a caller idempotency key.
...
incarnation, state `pending`, and time before the deadline. Target rejection changes it to
...
an ownership offer. Accepted relief creates one proposal-only grant for the current owner
epoch. A dead or stale target cannot accept and causes exactly one deduped reissue-required
...
| `release_responsibility_offers` and transition history | pending/terminal state and delivery coverage | purpose, exact target session key/incarnation, deadline, reason, predecessor, dedupe/reissue key, conditional relief expiry/scope, acknowledgments, actor/cause/time |
...
| `release_watch_facts`, heads, and append-only versions | head advance only | exact owner offer-or-epoch reference, durable selector, trigger class, acknowledged cursor, next due time, dedupe key, state, reissue policy, predecessor, actor/cause, sequence |
| `release_brief_revisions` and exact fact bindings | no mutation after commit | row-derived canonical bytes/digest, ordered fact-version ids, counts/hashes, provenance |
...
| `release_candidate_admissions` | prepare/effect/receipt/terminal phase and first outcome | trigger kind, event ids, derived or caller key, repository/ref/from/to SHA/generation, operation id |
...
`(trainId, lineGeneration, targetRef, fromSha, toSha)` key; a second index binds each event id
to one immutable envelope.
...
3. verifies that the brief's base names the exact protected-main anchor or a proved prior
   ...
   train;
4. creates the pending train with `main_snapshot` as the default mode and stores an optional
...
6. creates the `pending` `initial_owner` responsibility offer, acceptance action, and
   delivery coverage;
...
dedupe/reissue key, delivery acknowledgment state, actor, cause, and creation time. A relief
...
`pending` and time before the deadline; changes the offer to `accepted`; stores the terminal
actor/cause/time and acceptance acknowledgment; and satisfies its action. For
`initial_owner`, it opens the first epoch, acknowledges the unchanged initial brief,
activates the train, and installs the owner pointer. For `owner_transfer` or
`emergency_owner_transfer`, it atomically closes the prior epoch and opens the target epoch.
For `relief`, it creates one proposal-only grant with the offer's stored expiry/scope, bound
to the still-current owner epoch.
...
together. A non-null bundle requires a non-null releasable generation and resolves to that
generation. Candidate repository equals target
...
- `watches[]`: `{ "factId": <id>, "factVersionId": <id>,
  ...
  "nextDueAt": <timestamp|null>, "dedupeKey": <string>, "state": "active"|"retired",
  ...
  initial responsibility offer; an active or shipped brief uses its owner epoch. Decision
...
Every version stores its predecessor version or null, common record state `active | retired`,
...
due-time, state, and predecessor transition. A source transaction can use it only to append
...
a meaningful change and cannot stale a brief. Direct and parent evidence versions for one
...
acknowledged fact cursor, next due time, dedupe key, state, and reissue policy. The number
...
Each pending offer and stale brief generation has one action with a pending delivery or
...
```json
{"kind":"branch_push","eventId":"<provider-event-id>","operationId":"<operation-id>","idempotencyKey":"<derived-key>","repository":"github:owner/repo","ref":"refs/heads/release/<train-id>","lineGeneration":1,"fromSha":"<old-sha>","toSha":"<new-sha>"}
```
...
the line. The envelope to-SHA, line readback, and current generation SHA must match in one
admission decision. A mismatch returns `stale_candidate_head` and creates no candidate run.
Before that decision, the handler derives the idempotency key from
`(trainId, lineGeneration, targetRef, fromSha, toSha)` and inserts or reads its admission row.
The supplied idempotency key must equal that derivation, and the supplied operation id must
equal the row's operation id. The row appends each event id
and compares that event id's canonical payload hash. Its state advances through `prepared`,
`effect_pending`, `receipt_recorded`, and `terminal`; the CI invocation receipt is durable
before terminal publication. A replay before, during, or after those phases returns the same
operation and first run or non-run result and invokes CI zero additional times. A different
event id carrying the same derived key converges. Reusing one event id with another
operation, ref, from-SHA, or to-SHA returns `provider_event_identity_mismatch`. Explicit
dispatch inserts or reads one row keyed by owner session and idempotency key.
...
times, branch-push idempotency keys, and effect ids; it validates branch-push event and
operation ids against their immutable admission row. It strips caller-supplied audit fields.
...
relief grants, currency, watches/cursors, open actions, repository
...
The event stream exposes typed events for offer lifecycle, owner epoch, relief lifecycle,
...
refusal, generation supersession, CI admission/replay/refusal,
...
epoch exists. When the exact target accepts, then one epoch opens, those offer references
resolve to that epoch without rewriting the brief, and one acknowledgment binds the epoch to
the unchanged brief digest.
...
trigger, acknowledged cursor, next due time, dedupe key, state, and reissue policy. Given
...
**A-18C — Push operation and replay.** Given branch push event E1, operation O, and derived
key `(trainId, lineGeneration, ref, H, C)`, when the envelope carries that line generation
and key and E1 is delivered before prepare, during
`effect_pending`, after `receipt_recorded`, and after terminal publication, then one operation
advances through `prepared`, `effect_pending`, `receipt_recorded`, and `terminal`, and one
candidate run exists. Given E2 has the same tuple and operation O, it converges on that
result. Given the first delivery recorded a non-run refusal, every replay returns that
refusal and starts zero runs. Reusing E1 with another operation, ref, from-SHA, or to-SHA
returns `provider_event_identity_mismatch`.
...
event id and the same derived `(trainId, lineGeneration, targetRef, fromSha, toSha)` key, it
resolves to the first admission operation/result and preserves prepare/effect/receipt
history. Each external phase/target reuses one effect id.
...
owner, cause, fact/watch versions and cursors, anchor/protection digest, operation, event id,
derived key, expected/observed refs, generation-lock owner/state, generation eligibility,
bundle, digest, phase, outcome, retry state, and stale reason without reading prose notes.
...
acknowledges the current brief, and exposes no zero-or-two-owner snapshot. Given a retired
...
one current-epoch grant with E and that scope appears. When that session proposes a brief and evidence, the
proposal persists.
When it tries commit, acknowledge, decide, freeze, line-integrate, promote, tag, transfer,
...
operation cites either as revision 4 or revision 5 authority, then resolution returns
...
fact id/version and field exactly. Given both watch reissue policies and an active watch's
owner offer-or-epoch reference, selector, trigger, cursor, due time, and dedupe field,
row-derived order is stable. Given a
later retired watch version, it remains in history and a current brief that cites it is
refused. Given each

...
- **Identity descriptor**: a request-bound, authenticated, opaque capability
  ...
  binds one identity name to one state source and observation version without
  containing, selecting, opening, or decoding the identity item payload.
...
1. `StateResources.query_identity(source, {:metadata, name, request_binding})`
   returns `{:ok, identity_descriptor}` for every syntactically valid binary
   `name`, whether the name is present, absent, or forbidden to the resolved
   principal.
2. The caller evaluates `StateVisibility.identity_visible?/1`. A denial ends
   the request with the canonical `404 not_found` response and does not invoke
   hydration.
3. Only a permitted caller invokes
   `StateResources.query_identity(source, {:hydrate, identity_descriptor,
   request_binding})`. It returns `{:ok, hydrated_identity}`, `:not_found`,
   `:stale`, or `{:error, :invalid_identity_descriptor}`.
...
state source, source generation, request binding, and process generation that
issued it. A descriptor cannot hydrate kungfu, another identity name, another
database, or another request. Reuse within the same open bound operation is
idempotent while the observed row version is unchanged. The caller closes the
operation binding before it returns, responds, or enqueues output. Replay
under a different request binding or after that close is an invalid
descriptor, not a new lookup.

Authentication failure, malformed content, a wrong resource, a wrong source,
a wrong request binding, a closed operation, or cross-process replay returns
`{:error, :invalid_identity_descriptor}` before database access. The caller
records a loud internal `identity_descriptor_invalid` fault without the raw
descriptor, name-presence tag, or payload. A REST adapter emits the closed
`500 projection_invalid` error envelope and no partial response. Every invalid
descriptor case has that same response body and headers; descriptor validity
never becomes a name-existence answer.

#### Authorized hydration and row-version races

Hydration performs database work only after the shared visibility predicate
permits the operation. It uses the authenticated resource, name, source, and
observed version from the descriptor. Its canonical statement returns one
tagged outcome:
...
- REST identity detail mints a request binding, runs metadata, applies the
  shared visibility predicate, hydrates through the same query function when
  permitted, and passes only hydrated data to the pure serializer.
...
12. Given an authentic descriptor and descriptors with a changed byte, wrong
    resource, wrong name, wrong source, wrong source generation, wrong request
    binding, closed operation, prior process generation, and cross-request
    replay, when hydration runs, then only the authentic correctly bound
    descriptor may reach the database. Every invalid case returns
    `invalid_identity_descriptor`, records the safe loud fault, emits no
    descriptor content, and opens no payload. Repeating the authentic
    descriptor inside its bound operation returns the same result while the
    observed version remains unchanged.

...
Status: review-ready amendment after `att_f7e808e7-dcf3-4c25-8d2a-ff0ccad16eca`; targetless and unbound pending independent exact-revision re-review
...
Authority: `att_fae41482-848b-40c1-a8da-b1daf68e3c75`, `art_0b820dfb`, `att_08edcb9c-33d9-4039-bd3e-41562c965ec1`, `att_771854c7-8817-4540-8d73-43136199d673`, and review `att_f7e808e7-dcf3-4c25-8d2a-ff0ccad16eca`
...
- **Capture claim** is the binding generation and refresh identifier recorded when a full read starts. A completion shall publish only while its claim remains current.
...
9. **I-09 — Coalesced reads.** At most one full read shall be in flight for a binding generation. Concurrent refresh requests shall join that claim and shall not create another provider request.
...
The cache seam shall emit structured, public-safe events through Tightbeam's existing logger. An emitted event may contain only: event name, harness, public host name, local binding generation, capture source (`read` or `update`), outcome (`accepted`, `coalesced`, `superseded`, `invalid`, `timeout`, `provider_unavailable`, or `account_binding_unavailable`), projected freshness, and recognized window labels. At least one event per capture claim shall expose its terminal outcome. The implementation may suppress repeated `coalesced` events for later joiners to the same claim.
...
ADD wins because the existing consumer behavior depends on provider-backed Codex usage and Tightbeam currently omits it. DELETE would remove an already supported OpenClaw/Clawline surface. ACCEPT would preserve a demonstrated contract failure. The design therefore adds one adapter capture seam and one in-memory normalized cache. It deletes the tempting alternatives: raw-on-disk fixture capture, raw-response caching, a new route, persistent history, polling timers, direct Gateway credential access, inferred windows, special reset generation changes, emitted same-generation reset unavailability, and fabricated Claude data.
...
20. **A-20 — Observability (I-02, I-03).** Given capture scenarios that produce accepted, coalesced, superseded, invalid, timeout, provider-unavailable, and account-binding-unavailable outcomes, when each scenario runs, then every structured event contains only the allowed fields. Each capture claim has at least one terminal outcome among `accepted`, `superseded`, `invalid`, `timeout`, `provider_unavailable`, and `account_binding_unavailable`; a `coalesced` event alone does not satisfy that condition. Removing a claim's terminal outcome or adding a forbidden value fails the event-schema test.
...
25. **A-25 — Unavailable reason mapping (I-13, I-25).** Given four eligible bindings with no accepted capture, when the adapter respectively reports no ChatGPT account, provider refusal, existing request timeout, and a successful response with zero valid windows, then Tightbeam returns `account_binding_unavailable`, `provider_unavailable`, `timeout`, and `invalid_usage`, respectively, with `windows: []`. Given an accepted capture followed by terminal account invalidation, when session status is encoded, then Tightbeam changes `metadataContextGeneration`, clears the old windows, and returns `account_binding_unavailable` with `windows: []`. Given a same-generation accepted capture followed by provider refusal, timeout, or a response with zero valid windows, when session status is encoded, then Tightbeam retains stale windows and omits `unavailableReason`. Given reset observation with an accepted capture, when session status is encoded, then neither `stale_expired` nor `reset_elapsed` is emitted.
